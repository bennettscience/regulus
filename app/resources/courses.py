...
from app.static.assets.icons import registered, attended
...
class CourseListAPI(MethodView):
    ...
    def get(self: None) -> List[Course]:
        ...
        if args["format"] == "json":
            now = datetime.now()
            events = (
                Course.query.filter(Course.active == True, Course.starts >= now)
            ...
            if args["all"]:
                ...
                courses = Course.query.filter(
                    Course.active == True, Course.ends >= now
                ).all()
            ...
            if len(courses) > 0:
                ...
                for course in courses:
                    ...
                    if current_user.is_enrolled(course) and current_user.is_attended(
                        course
                    ):
                        course.state = "attended"
                        course.icon = attended
                    elif current_user.is_enrolled(
                        course
                    ) and not current_user.is_attended(course):
                ...
                return render_template(
                    "shared/partials/no-upcoming.html",
    ...
    def post(self: None) -> Course:
        ...
        body = {
            ...
            "attendees": [{"email": current_user.email, "responseStatus": "accepted"}],
        }
        ...
        if "conferenceData" in response.json():
            ...
            linktype_id = (
                CourseLinkType.query.filter(CourseLinkType.name == "Google Meet")
                .first()
                .id
            )
        ...
        response = make_response(render_template(template, events=schema.dump(events)))
        response.headers.set(
            "HX-Trigger",
            json.dumps({"showToast": "Successfully created {}".format(course.title)}),
        )
...
class CourseAPI(MethodView):
    def get(self: None, course_id: int) -> Course:
        ...
        """
        from app.static.assets.icons import clock, edit, pin, user
    ...
    def put(self: None, course_id: int) -> Course:
        ...
        if course is None:
            ...
            response.headers.set(
                "HX-Trigger", json.dumps({"showToast": "Nothing to update"})
            )
            return response
        else:
            ...
            try:
                ...
                ordered_regs = course.registrations.order_by(
                    CourseUserAttended.created_at
                ).all()
                if ordered_regs:
                    last_reg = course.registrations.order_by(
                        CourseUserAttended.created_at
                    )[-1].created_at
                ...
                response.headers.set(
                    "HX-Trigger",
                    json.dumps(
                        {"showToast": "Successfully updated {}".format(course.title)}
                    ),
                )
                return response
    ...
    def delete(self: None, course_id: int) -> dict:
        ...
        content = {"events": schema.dump(result)}

        response = make_response(render_template("admin/index.html", **content))
        response.headers.set(
            "HX-Trigger",
...
class CourseTypesAPI(MethodView):
    ...
    def post(self: None) -> CourseType:
        ...
        try:
            data = [
                {"value": coursetype.id, "text": coursetype.name}
                for coursetype in CourseType.query.all()
            ]
            response = make_response(
                render_template(
            ...
            response.headers.set(
                "HX-Trigger",
                json.dumps({"showToast": "Successfully added the event type."}),
            )
            return response
        except Exception as e:
            return jsonify(e)
...
class CoursePresentersAPI(MethodView):
    ...
    def post(self: None, course_id: int, *args: list) -> List[User]:
        ...
        response.headers.set(
            "HX-Trigger",
            json.dumps({"showToast": "Added {} as a presenter.".format(user.name)}),
        )
        return response
...
class CoursePresenterAPI(MethodView):
    ...
    def delete(self: None, course_id: int, user_id: int) -> List[User]:
        ...
        response.headers.set(
            "HX-Trigger",
            json.dumps({"showToast": "Successfully removed the presenter."}),
        )
        return response
...
class CourseAttendeesAPI(MethodView):
    ...
    def put(self: None, course_id: int, *args: list) -> List[User]:
        ...
        response = make_response(
            render_template(
                "admin/partials/registration-table.html", registrations=users
            )
        )
        response.headers.set(
            "HX-Trigger", json.dumps({"showToast": "All registrations updated"})
        )
        return response
    ...
    def post(self: None, course_id: int) -> List[User]:
        ...
        if ordered_regs:
            last_reg = course.registrations.order_by(CourseUserAttended.created_at)[
                -1
            ].created_at
            formatted_date = last_reg.strftime("%m/%d/%y, %I:%M %p")
        else:
        ...
        # Add some calculated stats about the event
        data = [
            {"label": "Registrations", "value": len(course.registrations.all())},
            {"label": "Last Registration", "value": formatted_date},
        ]
        ...
        response.headers.set(
            "HX-Trigger",
            json.dumps({"showToast": "Updated registrations successfully."}),
        )
        return response
        # return jsonify(UserAttended(many=True).dump(course.registrations))
...
class CourseAttendeeAPI(MethodView):
    def post(self: None, course_id: int) -> User:
        ...
        if course.available_size() > 0:
            ...
            if args["acc_required"]:
                ua = UserAccommodation(
                    required=args["acc_required"], note=args["acc_details"]
                )
                db.session.add(ua)
                db.session.commit()
        ...
        response.headers.set(
            "HX-Trigger",
            json.dumps(
                {"showToast": "Successfully registered for {}".format(course.title)}
            ),
        )
    ...
    def delete(self: None, course_id: int) -> dict:
        ...
        response.headers.set(
            ...
            json.dumps(
                {
                    "showToast": "Successfully cancelled registration for {}".format(
                        course.title
                    )
                }
            ),
